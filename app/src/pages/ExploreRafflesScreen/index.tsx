...
const ExploreRafflesScreen: FC = () => {
  ...
  const filterMap = useMemo(
    () => ({
      ...
      public: (raffle: Raffle) => raffle.metadata.name !== 'Unnamed Raffle',
    }),
    [publicKey]
  );
  ...
  const rafflesToShow = useMemo(() => {
    ...
    // HIDE raffle that not set in config
    toShow = toShow.filter(filterMap.public);
    return toShow;
  ...
  return (
    <>
      <Typography
        variant="h1"
        style={{ marginBottom: 10, fontSize: '2.25rem', fontWeight: 500 }}
      ...
      <Typography
        variant="subtitle1"
        style={{ marginBottom: 50, color: '#8c98a5' }}
      >
        Delysid Kiddos NFT Raffle
      </Typography>
      <FilterBar
